...
function list() {
  const [sortCriteria, setSortCriteria] = useState({ key: null, order: null }); // New state for sorting
...
useEffect(() => {
  ...
  const handlePropertySelect = (property) => {
    setSelectedProperty({ id: property.id, name: property.name });
    setSortCriteria({ key: null, order: null }); // Reset sort when selecting property type
    debouncedSearch(property.target.value);
  };
  ...
  const clearFilter = (filter) => {
    ...
    if (filter === 'city') {
      ...
      setSortCriteria({ key: null, order: null });
    ...
    if (newProjectId.trim()) {
      params.append('project', formatSlug(newProjectId.trim()));
    }
    navigate(`/search-Listing?${params.toString()}`);
  ...
  <Container>
    <Row className="blog-select mb-5">
      ...
      <DropdownButton
  id="Sort by"
  title={sortCriteria.key ? `Sort By: ${sortCriteria.key === 'tag_price' ? sortCriteria.order === 'asc' ? 'Price: Low to High' : 'Price: High to Low' : selectedProperty.name}` : 'Sort By'}
              variant="outline-light"
