...
const fetchEMIPlans = async () => {
  ...
  try {
    ...
    setError('Error fetching EMI plans: ' + (err.response?.data?.message || err.message));
  } finally {
    setLoading(false);
  }
};
      ...
      <Row className="justify-content-between p-0">
        <Col lg={7} md={12} className="bg-chane less-wi" data-aos="fade-right" data-aos-easing="ease-in-sine" data-aos-offset="300">
          <Form className="mt-4">
            <Form.Group className="mb-4">
              <Form.Label>
                Loan Amount (₹ {formatLoanAmount(loanAmount)} ) <span className="fw-bold"></span>
              </Form.Label>
              <Form.Range
                className="custom-range"
                min={1000000}
            ...
            <Form.Group className="mb-4">
              <Form.Label>
                Rate of Interest ({interestRate}%) <span className="fw-bold"></span>
              </Form.Label>
              <Form.Range
                className="custom-range"
                min={7}
